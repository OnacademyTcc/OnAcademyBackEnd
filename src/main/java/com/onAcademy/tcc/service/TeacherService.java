...
public class TeacherService {
	...
	public Teacher criarTeacher(TeacherDTO teacherDTO) throws MessagingException {
		...
		String emailText = "<!DOCTYPE html>" + "<html xmlns='http://www.w3.org/1999/xhtml'>" + "<head>"
				...
				+ "  .divider { border-top: 2px dashed #a8d0ff; margin: 25px 0; }" + "</style>" + "</head>" + "<body>"
				+ "<div class='container'>" + "  <div class='logo'>ON Academy</div>" + "  <h1>Olá, "
				+ savedTeacher.getNomeDocente() + "! 👋</h1>" + "  <div class='info-box'>"
				...
				+ "    <p style='font-size: 15px; margin: 10px 0;'><strong>🔑 Código de Matrícula:</strong> "
				+ savedTeacher.getIdentifierCode() + "</p>"
				+ "    <p style='font-size: 15px; margin: 10px 0;'><strong>🔒 Senha temporária:</strong> " + rawPassword
				+ "</p>" + "  </div>" + "  <div style='text-align: center;'>"
				+ "    <a href='https://www.onacademy.com.br/' class='login-btn'>ACESSAR PLATAFORMA</a>" + "  </div>"
				+ "  <div class='divider'></div>"
				+ "  <p style='font-size: 14px;'>Se tiver qualquer dúvida ou precisar de ajuda, entre em contato conosco:</p>"
				+ "  <p style='font-size: 14px;'>📧 onacademy.tcc@gmail.com</p>" + "  <div class='footer'>"
	...
	public Teacher atualizarTeacher(Long id, Teacher teacher) {
		...
		if (existingTeacher.isPresent()) {
			...
			if (teacher.getImageUrl() != null && !teacher.getImageUrl().isEmpty()
					&& teacher.getImageUrl().matches("^http://.*")) {
			...
			String emailText = "<!DOCTYPE html>" + "<html xmlns='http://www.w3.org/1999/xhtml'>" + "<head>"
					...
					+ "  .warning { color: #d32f2f; font-weight: bold; }" + "</style>" + "</head>" + "<body>"
					+ "<div class='container'>" + "  <div class='logo'>ON Academy</div>" + "  <h1>Olá, "
					+ atualizarTeacher.getNomeDocente() + "!</h1>" + "  <div class='alert-box'>"
					...
					+ "    <p style='font-size: 15px; margin: 12px 0;'><strong>🔑 Código de Matrícula:</strong> "
					+ atualizarTeacher.getIdentifierCode() + "</p>"
					+ "    <p style='font-size: 15px; margin: 12px 0;'><strong>🔒 Nova Senha:</strong> " + rawPassword
					+ "</p>" + "  </div>" + "  <div style='text-align: center;'>"
					...
					+ "  <div class='footer'>" + "    <p>Atenciosamente,<br><strong>Equipe ON Academy</strong></p>"
					+ "    <p style='font-size: 12px; color: #999;'>Este é um e-mail automático, por favor não responda.</p>"
					+ "  </div>" + "</div>" + "</body>" + "</html>";
			try {
